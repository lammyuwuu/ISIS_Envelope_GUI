...
def getBetaValues(twiss) -> pd.DataFrame:
    ...
    # Check that arrays have same length
    assert(len(betx_array) == len(bety_array))
    assert(len(time_array) == len(bety_array))
    assert(len(time_array) == len(betx_array))

    # Calculating the currents
    tq_currents_df = tune_di_df(Qh=betx_array, Qv=bety_array, baseQh=4.331, baseQv=3.731, time_array=time_array, z=np.array([-4.73e-3, -5.99E-03, 4.45E-03, 2.40E-03]))
    #print(tq_currents_df)
    print('Expected values found')
