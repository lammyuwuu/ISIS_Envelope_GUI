...
def cpymad_plot_CO_plotly(madx_instance, df_myTwiss, sequence_name, save_file=None, xlimits=None, ylimits=None, ptc_twiss=False):
    if ptc_twiss:
        gamma_key = 'GAMMA'; pc_key='PC'
        ptc_twiss_read_Header = dict(df_myTwiss.headers)
        gamma_rel = ptc_twiss_read_Header[gamma_key]
    ...
    plot_title = f"{sequence_name} Q1={qx:.3f}, Q2={qy:.3f}"
    ...
    # --- Betas and Dispersions ---
    fig.add_trace(go.Scatter(x=df_myTwiss['s'], y=df_myTwiss['betx'],
                             mode='lines', name='βₓ', line=dict(color='blue'),showlegend = True, legendgroup='βₓ'), row=1, col=1, secondary_y=False)
    fig.add_trace(go.Scatter(x=df_myTwiss['s'], y=df_myTwiss['bety'],
                             mode='lines', name='βᵧ', line=dict(color='red'), showlegend = True, legendgroup='βᵧ'), row=1, col=1, secondary_y=False)

    if ptc_twiss:
        dx = df_myTwiss['disp1'] / beta_rel
        dy = df_myTwiss['disp3'] / beta_rel
    else:
        dx = df_myTwiss['dx']
        dy = df_myTwiss['dy']

    fig.add_trace(go.Scatter(x=df_myTwiss['s'], y=dx,
                             mode='lines', name='Dₓ', line=dict(color='green'),showlegend = True, legendgroup='Dₓ'), row=1, col=1, secondary_y=True)
    fig.add_trace(go.Scatter(x=df_myTwiss['s'], y=dy,
                             mode='lines', name='Dᵧ', line=dict(color='purple'),showlegend=True, legendgroup = 'Dᵧ'), row=1, col=1, secondary_y=True)

    # --- Closed Orbit X ---
    fig.add_trace(go.Scatter(x=df_myTwiss['s'], y=df_myTwiss['x']*1e3,
                             mode='lines', name='Horizontal Closed Orbit', line=dict(color='black')), row=2, col=1)

    # --- Closed Orbit Y ---
    fig.add_trace(go.Scatter(x=df_myTwiss['s'], y=df_myTwiss['y']*1e3,
                             mode='lines', name='Vertical Closed Orbit', line=dict(color='black')), row=3, col=1)

    # Axis configuration
    fig.update_layout(title=plot_title, height=800, width=1000,
                      legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))

    fig.update_yaxes(title_text='βₓ, βᵧ [m]', row=2, col=1, secondary_y=False)
    fig.update_yaxes(title_text='Dₓ, Dᵧ [m]', row=2, col=1, secondary_y=True, color='green')
    fig.update_yaxes(title_text='x [mm]', row=3, col=1)
    fig.update_yaxes(title_text='y [mm]', row=4, col=1)
    fig.update_xaxes(title_text='s [m]', row=4, col=1)
    ...
    if ylimits:
        fig.update_yaxes(range=ylimits, row=3, col=1)
        fig.update_yaxes(range=ylimits, row=4, col=1)
...
def cpymad_plot_CO_sp_plotly(madx_instance, df_myTwiss, sequence_name, save_file=None, xlimits=None, ylimits=None, ptc_twiss=False):
    if ptc_twiss:
        gamma_key = 'GAMMA'; pc_key = 'PC'
        ptc_twiss_read_Header = dict(df_myTwiss.headers)
        gamma_rel = ptc_twiss_read_Header[gamma_key]
        beta_rel = np.sqrt(1. - (1. / gamma_rel ** 2))
        p_mass_GeV = 0.93827208816
        tot_energy = gamma_rel * p_mass_GeV
        kin_energy = tot_energy - p_mass_GeV
        momentum = ptc_twiss_read_Header[pc_key]

        print(f'Relativistic Gamma = {round(gamma_rel, 3)}')
        print(f'Relativistic Beta = {round(beta_rel, 3)}')
        print(f'Total Energy = {round(tot_energy, 4)} GeV')
        print(f'Kinetic Energy = {round(kin_energy * 1E3, 3)} MeV')
        print(f'momentum = {round(momentum, 3)} GeV/c')
    ...
    fig = make_subplots(
        ...
        row_heights=[0.35, 0.25, 0.25],
        subplot_titles=[plot_title, '', '']
    )

    # Beta functions
    fig.add_trace(go.Scatter(x=df_myTwiss['s'], y=df_myTwiss['betx'], name='βx', line=dict(color='blue')), row=1, col=1)
    fig.add_trace(go.Scatter(x=df_myTwiss['s'], y=df_myTwiss['bety'], name='βy', line=dict(color='red')), row=1, col=1)

    # Dispersions
    if ptc_twiss:
        dx, dy = df_myTwiss['disp1'] / beta_rel, df_myTwiss['disp3'] / beta_rel
    else:
        dx, dy = df_myTwiss['dx'], df_myTwiss['dy']
    fig.add_trace(go.Scatter(x=df_myTwiss['s'], y=dx, name='Dx', line=dict(color='green')), row=1, col=1)
    fig.add_trace(go.Scatter(x=df_myTwiss['s'], y=dy, name='Dy', line=dict(color='purple')), row=1, col=1)

    # Horizontal CO
    fig.add_trace(go.Scatter(x=df_myTwiss['s'], y=df_myTwiss['x'] * 1e3, name='Horizontal CO', line=dict(color='black')), row=2, col=1)

    # Vertical CO
    fig.add_trace(go.Scatter(x=df_myTwiss['s'], y=df_myTwiss['y'] * 1e3, name='Vertical CO', line=dict(color='black')), row=3, col=1)

    # SP lines and labels
    for i in range(10):
        x_pos = 16.336282 * i
        for r in [2, 3]:
            fig.add_vline(x=x_pos, line=dict(color='gray', dash='dash', width=0.8), row=r, col=1)
            fig.add_annotation(x=x_pos + 8, y=8, text=f'SP{i}', showarrow=False,
                               row=r, col=1, font=dict(size=8, color='blue'))
    ...
    if ylimits and len(ylimits) == 2:
        ...
        min_co = min(np.min(df_myTwiss['x']), np.min(df_myTwiss['y'])) * 1e3
        max_co = max(np.max(df_myTwiss['x']), np.max(df_myTwiss['y'])) * 1e3
        co_min = np.floor(min_co / 10) * 10
        co_max = np.ceil(max_co / 10) * 10

    # Layout updates
    fig.update_yaxes(title_text='β / D [m]', row=2, col=1)
    fig.update_yaxes(title_text='x [mm]', range=[co_min, co_max], row=3, col=1)
    fig.update_yaxes(title_text='y [mm]', range=[co_min, co_max], row=4, col=1)
    fig.update_xaxes(title_text='s [m]', row=4, col=1)
    fig.update_layout(height=850, width=1000, title_text=plot_title, showlegend=True)
